	...
	public class TileManager {
	    ...
	    public int[][] m_mapTileNum; // Déclarer m_mapTileNum avec une visibilité protected
	    public Tile[] m_tile; // Déclarer m_tile avec une visibilité protected
	    int m_maxTiles = 10;      //nombre maximum de tiles chargeable dans le jeu
	    private BufferedImage m_backgroundImage;  // Image de fond
		public Obstacle[] m_obstacles; // Ajout des obstacles
	    ...
	    public TileManager(GamePanel gp) {
	        m_mapTileNum = new int[gp.MAX_SCREEN_COL][gp.MAX_SCREE_ROW]; // Initialiser m_mapTileNum avec les dimensions appropriées
	
	        this.m_gp = gp;
	        m_tile = new Tile[m_maxTiles];
	        m_mapTileNum = new int[gp.MAX_SCREEN_COL][gp.MAX_SCREE_ROW];
			m_obstacles = new Obstacle[3]; // Initialisation du tableau d'obstacles
	        this.getTileImage();
	        this.loadMap("/maps/map.txt");
			this.initializeObstacles(); // Initialisation des obstacles
	    }
	    ...
	    public void getTileImage() {
	        try {
	            ...
	            m_tile[1].m_collision = true; // Définir m_collision à true pour BRICK2
		...
		public void initializeObstacles() {
			// Initialisation des obstacles à partir de la carte
			int numObstacles = 0;
			for (int x = 0; x < m_mapTileNum.length; x++) {
				for (int y = 0; y < m_mapTileNum[x].length; y++) {
					if (m_mapTileNum[x][y] == 2) { // Si c'est un obstacle dans la carte (à adapter selon votre configuration)
						if (numObstacles < m_obstacles.length) {
							BufferedImage obstacleImage = loadImage("/enemy/enemy.jpg");
							m_obstacles[numObstacles] = new Obstacle(x * m_gp.TILE_SIZE, y * m_gp.TILE_SIZE, obstacleImage);
							numObstacles++;
						} else {
							break; // Sortir de la boucle si tous les obstacles ont été initialisés
						}
					}
				}
			}
